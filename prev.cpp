...
unsigned int convertTo_UINT(int x) {
...
void createBranch(unsigned int val, shared_ptr<Node> current,
  ...
  unsigned int mid =
      (unsigned int)(((long long int)current->x + (long long int)current->y) /
          2);
...
void pushBack(int value) {
  unsigned int x = convertTo_UINT(value);
...
int prevInRange(int i, int lo, int hi) {
  auto lo2 = convertTo_UINT(lo);
  auto hi2 = convertTo_UINT(hi);
  found = false;
  auto ans = search(lo2, hi2, roots[i]);
